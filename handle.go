...
func handle(data *string, conn *net.Conn, timeout *time.Duration, last_will *string, last_will_p *string, username *string) bool {
	defer func() {
	...
	if "MQS" == (*data)[0:3] {
		...
		if act == '0' {
			...
			done, resp := lastpublish(data, last_will, last_will_p, username)
			if resp != ""{
				_,err :=(*conn).Write([]byte(resp))
				if err != nil{}
			}
